			  ...
			  struct vb2_v4l2_buffer *dst_vb)
{
	...
	if (ctx->is_stateless) {
		...
		if (ret) {
			pr_err("%s: v4l2_ctrl_request_complete returned %d\n", __func__, ret);
			return ret;
		}
		...
		if (ntohl(ctx->state.header.flags) & FWHT_FL_P_FRAME) {
			...
			if (ref_vb2_buf->state == VB2_BUF_STATE_ERROR) {
				pr_err("%s: propagating error of ref frame\n", __func__);
				ret = -EINVAL;
			}
	...
	if (ctx->is_enc) {
		...
		if (comp_frame_size > ctx->comp_max_size) {
			pr_err("%s: comp_frmame_size too big\n", __func__);
			return -EINVAL;
		}
...
static int vicodec_try_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case VICODEC_CID_STATELESS_FWHT:
		...
		if (!validate_stateless_params_flags(params, q_dst->info)) {
			pr_err("%s info from params is wrong\n", __func__);
			return -EINVAL;
		}
		if (params->comp_frame_size > total_frame_size(q_dst)) {
			pr_err("%s comp size %u, max alowed %u\n", __func__, params->comp_frame_size,
					total_frame_size(q_dst));
			return -EINVAL;
		}
	default:
		return 0;
